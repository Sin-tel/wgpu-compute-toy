...
var<private> spectrum: array<vec3<f32>, SPECTRUM_LEN> = array(
...
fn main_image(@builtin(global_invocation_id) id: uint3) {
    ...
    col = col * pow(2.0, custom.exposure*0.5);
    ...
    // clear
    atomicStore(&hist[id.x][id.y][0], 0u);
    atomicStore(&hist[id.x][id.y][1], 0u);
    atomicStore(&hist[id.x][id.y][2], 0u);
}
